...
class MLP2(Module):
    ...
    def __init__(self):
        ...
        self.layers = Sequential(
            Flatten(),
            Linear(in_features=30*30, out_features=128),            #hidden layer 1 (128 units)    
            ReLU(),                 
            Linear(in_features=128, out_features=64),               #hidden layer 2; linear units
            ReLU(),
            Linear(in_features=64, out_features=10),                #narrow to 10 logits
            LogSoftmax(dim=1)
            )
...
class CNN(Module):
    ...
    * A layer of 10 fully-connected linear units with log-softmax activation (the output layer)
    ...
    def __init__(self):
        super().__init__()
        self.layers = Sequential(
            #no flatten because this is convolutional
            Conv2d(in_channels=1, out_channels=32, kernel_size=5, stride=1), #grayscale channel (1) not rgb (3)
            ReLU(),
            ...
            Flatten(),
            Linear(in_features=64*4*4, out_features=512),                    #to match the matrice sizes 
            ReLU(),
