import numpy as np
import cv2 as cv

haar_cascade = cv.CascadeClassifier('haar_face.xml')


#fetch files we made from the previous file
people = ['Ben Afflek', 'Elton John', 'Jerry Seinfield', 'Madonna', 'Mindy Kaling']
#features = np.load('features.npy', allow_pickle = True)
#labels = np.load('labels.npy')

face_recognizer = cv.face.LBPHFaceRecognizer_create() #we are fetching a pre-built model
face_recognizer.read('face_trained.yaml')


#---- all initial things are done ----

#fetch a validation image
img = cv.imread('Faces/val/ben_afflek/2.jpg')
gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
cv.imshow('Unidentified Person', gray)

#detect the face in the image
faces_rect = haar_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=4)
for (x,y,w,h) in faces_rect:
    face_roi = gray[y:y+h, x:x+w]
    label, confidence = face_recognizer.predict(face_roi)
    print(f'Label = {people[label]} with a confidence of {confidence}') 

    cv.putText(img, str(people[label]), (20,20), cv.FONT_HERSHEY_COMPLEX, 1.0, color=(0,255,0), thickness=2)
    cv.rectangle(img, (x,y), (x+w, y+h), (0,255,0), 2)

cv.imshow('detected face', img)
cv.waitKey(0)